...
class PatchSplitter:
    ...
    def split_queries(self, queries, t):
        ...
        transforms = torch.tensor([[0, 0, 0], [0, a, 0], [0, W-w, 0], [0, 0, H-h], [0, a, H-h], [0, W-w, H-h]], device=queries.device)
        patch_queries = pad_query.repeat(6, self.max_patch_queries, 1)
        for i in range(6):
            patch_queries[i, :self.tracks_mask[i].sum(), :] = queries[0, self.tracks_mask[i], :] - transforms[i]
